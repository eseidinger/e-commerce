...
public class JwtUtils {
  ...
  /**
   * Verifies a JWT using the public key from Keycloak, and extracts username and roles.
   *
   * @param token The JWT string
   * @param keycloakPublicKey The Keycloak public key in Base64 (from realm settings -> Keys ->
   *     RS256)
   * @return Claims
   */
   ...
   */
  public static TokenInfo verifyAndExtract(String token, String jwksUrl, String kid)
      throws Exception {
    PublicKey publicKey = getPublicKeyFromJwks(jwksUrl, kid);
  ...
  public static PublicKey getPublicKeyFromJwks(String jwksUrl, String kid) throws Exception {
    ...
    String jwksJson = client.target(jwksUrl).request("application/json").get(String.class);
    client.close();
    ObjectMapper mapper = new ObjectMapper();
